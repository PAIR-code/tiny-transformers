...
// https://colab.reseach.google.com/drive/1NZbBUxVpqDeKvl6m-gVtFf_ZwxPChYuc?resourcekey=0-M2Ac8ikYbWV9TBjxbFtTCQ#scrollTo=X8GQoTH-5m_y
...
window.initGrid = function(renderAll){
  ...
  function render(){
    ...
    positions.forEach(d => {
      d.logits = visState?.logits[d.seqIndex]
      d.softmax = util.softmax(d.logits)
    })
    
    logitLinesSel.appendMany('path', positions)
      .translate([s/4, -s/4])
      .at({
        d: d => ['M', pos2px(centerPos(d)), 'h', (s/2)*d.softmax[dataset_config.pad_token-2]].join(' '),
        stroke: 'red',
        strokeWidth: 3,
      })
    logitLinesSel.appendMany('path', positions)
      .translate([s/4, -s/4])
      .at({
        d: d => ['M', pos2px(centerPos(d)), 'v', -(s/2)*d.softmax[dataset_config.pad_token-1]].join(' '),
        stroke: 'steelblue',
        strokeWidth: 3,
      })
  }
...
window.init = async function(){
  ...
  async function renderAll(){
    ...
    visState.logits = await util.getPythonFile('seq_to_logits.json', visState.seq)
