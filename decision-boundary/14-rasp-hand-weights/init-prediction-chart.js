...
window.initPredictionChart = function(model){
  ...
  async function render(){
    const lastPos = _.last(modelInput.curInput)
    const batch = d3.range(model.hyper.n_num_tokens).map(i => {
      const rv = modelInput.curInput.slice()
      rv[model.hyper.sequence_length - 1] = i
      return rv
    })

    const leftPredictions = model.tidy(batch)
      .slice([0, 6, model.hyper.n_num_tokens], [model.hyper.n_num_tokens, 1, 1])
      .dataSync()
