...
window.initOvSimple = function(model){
  ...
  function drawCircuit(kqCircuit, headIndex){
    ...
    util.ggPlot(c, false)
    util.addAxisLabel(c, 'Key Token', 'Dot Product w/ L & R Token Embeddings', 28)
    ...
    d3.select('.lr-color').html('')
      ...
      .text(d => d)
      .st({background: (d, i) => color.lr[i], color: '#fff', padding: 4, margin: 5,  display: 'inline-block'})

    var lineSel = c.svg.appendMany('path', d3.nestBy(dataLR, d => d.key))
      .at({strokeWidth: 1})
      .translate(d => xPos(d.key), 0)

    var circleSel = c.svg.appendMany('circle', dataLR)
      .translate(d => xPos(d.key), 0)
      .at({r: d => d.key > 99 ? 4 : 2, fill: d => color.lr[d.isLeftOutput]})
    ...
    function render(){
      ...
      circleSel.at({cy: d => c.y(d.v)})

      lineSel.at({
        d: d => `M 0 ${c.y(d[0].v)} V ${c.y(d[1].v)}`,
