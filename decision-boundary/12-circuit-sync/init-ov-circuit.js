...
window.initOvCircuit = function(model){
  ...
  function drawCircuit(kqCircuit, headIndex){
    ...
    d3.drawAxis(c)
    c.svg.select('.x').translate(c.height + pad*6 + 5, 1)

    util.addAxisLabel(c, 'Key Token', 'Output Token Embedding', 28)
    ...
    function render(){
      ...
      grid.forEach((row, i) => {
        row.forEach((v, j) => {
          ...
          if (j == 100) dj += pad
          if (j == 101) dj += pad*2
          if (j >= 100) width = pad*2 - 1

          var di = i
          var height = s
          if (i == 100) di += pad
          if (i == 101) di += pad*2
          if (i >= 100) height = pad*2 - 1 

          ctx.beginPath()
          ctx.fillStyle = color.divScale(v)
          ctx.rect(di*s, dj*s, height, width)
          ctx.fill()
        })
      })
    }
