...
- \[optional\] Install globally the Angular 18 CLI with `$ pnpm add -g @angular/cli` so that you can directly use `ng` commands, otherwise you have to use `pnpm dlx ng`.
...
Start a dev sever with `pnpm start`.
...
```sh
pnpm dlx ts-node src/weblab-examples/build.script.ts --mode=serve
```
