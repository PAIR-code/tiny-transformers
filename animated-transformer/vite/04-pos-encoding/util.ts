...
function initVocabConfig(config) {
  ...
  const tokenEmb = new TokenEmb(vocab, makeTruncNormal(
    { token: vocab.length, inputRep: config.decoderSizes.rep }));
...
function trainStep(
  ...
  function tfLoss(...tensors: tf.Tensor[]): tf.Tensor {
    ...
    const loss = transformer.transformerLastTokenCrossEntropyLoss(
      decoderComputation, config.vocab.tokenEmb.embeddings, batch.targets);
...
function generateStaticBatch(batchId, config) {
  ...
  var examples = d3.range(Math.pow(config.taskConfig.inputSeqLen, 2)).map(i => {
    ...
    var input = d3.range(config.taskConfig.inputSeqLen)
      .map(i => str[i] == '1' ? 'b' : 'a')
...
function addCanvas(sel, width, height) {
  ...
  var canvasSel = sel.st({ position: 'relative', width, height })
    .append('canvas')
  ...
  var svgSel = sel.append('svg.svg-drag')
    .at({ width, height })
...
export { initVocabConfig, addCanvas, trainStep, flattenLayerParams, generateStaticBatch, transformerSoftmaxAndLoss }
