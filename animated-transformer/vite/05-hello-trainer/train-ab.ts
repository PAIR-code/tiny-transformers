...
export function trainAB() {
  ...
  const trainingBatches = [
    ...generateBatches(nBatches, taskConfig.batchSize, (_) => abtask.genRandExample(taskConfig)),
  ].map((batch) => {
    ...
    const batchedOutputEmb = new GTensor(
      tf.tensor(
  ...
  function trainStep(
    ...
    function tfLoss(...tensors: tf.Tensor[]): tf.Tensor {
      ...
      const decoderComputation = transformer.computeTransformer(initParams, batch.inputs);
...
function plotParams(loss, params, batch) {
  ...
  var layerSel = sel.appendMany('div.layer', params.layers).each(drawLayer);
}

function drawLayer(layer, i) {
  var sel = d3.select(this);

  var matrices = _.sortBy(d3.entries(layer), (d) =>
    ['keyM', 'queryM', 'valueM', 'ff1', 'ff2'].indexOf(d.key)
  );
  sel.appendMany('div', matrices).each(drawMatrix).st({ display: 'inline-block', padding: 2 });
  ...
  function drawMatrix({ key, value }) {
    ...
    for (var i = 0; i < rawTensor.length; i++) {
      ...
      ctx.beginPath();
      ctx.fillStyle = rawTensor[i] == 0 ? '#f0f' : d3.interpolatePuOr(rawTensor[i] * 2 + 0.5);
