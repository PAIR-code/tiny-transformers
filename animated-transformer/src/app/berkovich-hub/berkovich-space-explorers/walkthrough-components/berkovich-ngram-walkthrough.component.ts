...
import { Rational, formatDigitSequence, formatRational } from '../../../../lib/berkovich/berkovich';
...
@Component({
  ...
  template: `
    ...
    <p style="margin: 0 0 12px 0; font-size: 12.5px; color: #475569; line-height: 1.5;">
      This model aggregates a context of up to $N$ preceding characters to predict the next character using Berkovich spaces.
    </p>
    ...
    @if (details(); as walkthrough) {
      ...
      <div style="display: flex; flex-direction: column; gap: 16px; font-size: 13px; line-height: 1.5; color: #334155;">
              ...
              <div style="display: flex; flex-direction: column; gap: 6px;">
                @for (h of walkthrough.aggregated; track h.dim) {
                  <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 4px; padding: 8px 12px;">
                    <div style="font-weight: 700; color: #0f172a; margin-bottom: 4px;">Dimension {{ h.dim }}</div>
                    
                    <div style="display: flex; flex-wrap: wrap; gap: 16px; align-items: center; font-family: monospace;">
                      <div style="display: flex; flex-direction: column; gap: 2px;">
                        <span style="font-size: 10px; color: #64748b;">Center c_H (Rational)</span>
                        <span style="font-weight: 600;">{{ formatRationalVal(h.center) }}</span>
                      </div>
                      <div style="display: flex; flex-direction: column; gap: 2px; min-width: 140px;">
                        <span style="font-size: 10px; color: #64748b;">Center c_H (p-adic digits)</span>
                      ...
                      <div style="display: flex; flex-direction: column; gap: 2px;">
                        <span style="font-size: 10px; color: #64748b;">Log-radius &rho;_H</span>
                        <span style="font-weight: 600; color: #2563eb;">{{ h.rho?.toFixed(4) }}</span>
                      </div>
                    </div>
                  </div>
                }
        ...
        <div>
          ...
          <details style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 6px; overflow: hidden; margin-top: 8px;">
            <summary style="padding: 8px 12px; font-size: 12px; font-weight: 600; color: #3b82f6; cursor: pointer; background: #f8fafc; user-select: none;">
              Show Step 3 projection & path loss details
            </summary>
                ...
                <table style="width: 100%; border-collapse: collapse; text-align: left;">
                  <thead>
                    <tr style="background: #f8fafc; border-bottom: 1px solid #e2e8f0; position: sticky; top: 0; z-index: 10;">
                      ...
                      <th style="padding: 8px 12px; font-size: 11px; font-weight: 600; color: #475569;">Dimension Details & Path Loss calculation</th>
                      <th style="padding: 8px 12px; font-size: 11px; font-weight: 600; color: #475569; text-align: right;">Final Logit Score (D)</th>
                    </tr>
                  </thead>
                  <tbody>
