              ...
              <div style="display: flex; flex-direction: column; gap: 12px; margin-top: 10px; max-height: 250px; overflow-y: auto; padding-right: 4px;">
                ...
                @for (r of dimensions(); track r) {
                  @for (c of dimensions(); track c) {
                    <div style="display: flex; gap: 8px; align-items: center; margin-bottom: 6px; font-size: 11px;">
                      ...
                      <input type="text" [ngModel]="getMStr(r, c)" (ngModelChange)="updateM(r, c, $event)" style="width: 60px; border: 1px solid #cbd5e1; border-radius: 4px; padding: 2px 4px;">
                    </div>
                  }
                }
                ...
                @for (r of dimensions(); track r) {
                  <div style="display: flex; gap: 8px; align-items: center; margin-bottom: 6px; font-size: 11px;">
                    ...
                    <input type="text" [ngModel]="getBStr(r)" (ngModelChange)="updateB(r, $event)" style="width: 60px; border: 1px solid #cbd5e1; border-radius: 4px; padding: 2px 4px;">
                  </div>
                }
              </div>
            </details>
...
export class PadicLinearModelInspectorToolComponent {
  ...
  updateM(r: number, c: number, valStr: string) {
    try {
      const val = parseToRational(valStr);
      const key = `${r}-${c}`;
