...
function updateChart(drawData: DrawData, chartParts: ChartParts) {
  ...
  const {
    ...
    xAxis, yAxis, yAxisLines, yLabel, strokeWidth, strokeLinecap,
    strokeLinejoin, strokeOpacity,
    legendX, legendY,
    curveFn,
  } = drawData;

  // Construct a line generator.
  const line = d3.line<NamedChartPoint>(
    p => p.x,
    p => p.y)
    .curve(curveFn)
    .x(p => xScale(p.x))
  ...
  legend.selectAll('circle')
    ...
    .join('circle')
    .attr('cx', legendX)
    .attr('cy', function (d, i) { return legendY + i * 15 })
    .attr('r', 4)
  ...
  legend.selectAll('text')
    ...
    .attr('x', legendX + 10)
    .attr('y', function (d, i) { return legendY + i * 15 })
    .style('fill', function (d) { return color(d) })
...
function initDrawData(config: LineChartConfig, data: NamedChartPoint[])
  : DrawData {
