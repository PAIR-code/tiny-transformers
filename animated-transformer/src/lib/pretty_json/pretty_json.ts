...
// ----------------------------------------------------------------------------
export function tubeifyJsonValue(value: JsonValue)
  : Tube {
  switch (typeof value) {
    case 'string': return new LeafTube(value);
    case 'number': return new LeafTube(value);
    case 'boolean': return new LeafTube(value);
    case 'object':
      // null case.
      if (!value) {
...
export function stringifyJsonValue(o: JsonValue, config?: StringifyConfig)
  : string {
  if (!config) {
    config = {
      curIndent: '',
