...
export type DictArrTree<LeafT> = (
  LeafT | LeafT[] | DictTree<LeafT> | DictTree<LeafT>[]);
...
export class JsTreeLib<LeafT> {
  constructor(
    public isLeaf: (t: unknown) => t is LeafT,
  ) { }
  ...
  *iter<T extends LeafT>(t: DictArrTree<LeafT>): Generator<T, undefined, undefined> {
    ...
    } else { // t: ParamsDict<T> | ParamsDict<T>[]
  ...
  map<T, TreeKind extends DictArrTree<LeafT>>(
    t: TreeKind, fn: (l: LeafT, i: number) => T): DictArrTree<T> {
    let i = 0;
    function mapWithIndex(l: LeafT): T {
      return fn(l, i++);
    }
  ...
  reduce<T>(
    ...
    initialValue: T, t: DictArrTree<LeafT>
  ): T {
...
export class JsTree<LeafT, ObjT> {
  ...
  // Assumes they have the same shape.
  forEachZip<Leaf2 extends LeafT>(fn: (l1: LeafT, l2: Leaf2, i: number) => void,
    t2: JsTree<Leaf2, unknown>
  ): void {
  ...
  copyFromFlattened(
    ...
    return new JsTree(this.lib, unflattened as ObjT & DictArrTree<LeafT>);
  }
}
