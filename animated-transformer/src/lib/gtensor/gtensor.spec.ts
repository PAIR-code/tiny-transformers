...
describe('gtensor', () => {
  ...
  it('transposeTo', async () => {
    ...
    const g2 = new gtensor.GTensor(tf.tensor(
      [[[1, 2], [1, 2], [1, 2], [1, 2]], // pos = 1
      [[3, 4], [3, 4], [3, 4], [3, 4]], // pos = 2
      [[5, 6], [5, 6], [5, 6], [5, 6]]]  // pos = 3
    ), ['pos', 'example', 'repSize']);
    ...
    tf.test_util.expectArraysClose(
      likeg2.tensor.dataSync(), g2.tensor.dataSync());
  });
  ...
  it('pointwiseAdd', async () => {
    const bar = new gtensor.GTensor(tf.tensor(
      [[1, 2], [3, 4]]
    ), ['example', 'repSize']);
    const foo = new gtensor.GTensor(tf.tensor(
      [[1, 2], [3, 4], [5, 6]]
    ), ['point_id', 'repSize']);
  ...
  it('pointwiseAdd2', async () => {
    ...
    const r = bar.pointwiseAdd(foo);
    expect(r.gshape()).toEqual({ example: 2, batch: 1, point_id: 3, repSize: 2 });
  ...
  it('prodOverDims', () => {
    ...
    tf.test_util.expectArraysClose(rSumAB.tensor.dataSync(),
      [0, 0]);
  });
  ...
  it('gather', () => {
    // Making a GTensor from a tensor by naming the dimensions:
    const g = new gtensor.GTensor(tf.tensor(
      [[1, 2, 3, 4], [5, 6, 7, 8]],
    ), ['heads', 'relativePos']);

    const indexes = new gtensor.GTensor(
      tf.tensor(
        [[0, 1], [1, 2], [2, 3]], [3, 2], 'int32'),
      ['keyPos', 'queryPos']);
    ...
    tf.test_util.expectArraysClose(
      ...
      tf.tensor(
        [[[1, 2], [2, 3], [3, 4]],
        [[5, 6], [6, 7], [7, 8]]],).dataSync()
    );
  });

  it('variable assign', () => {
    // Making a GTensor from a tensor by naming the dimensions:
    const x = new gtensor.GTensor(tf.tensor(
      [1, 2, 3, 4],
    ), ['foo']);

    const x2 = new gtensor.GTensor(tf.tensor(
      [0, 0, 0, 0],
    ), ['foo']);
  ...
  it('softmax', () => {
    ...
    const g = new gtensor.GTensor(tf.tensor(
      ...
      [[2, 3.1], [3, 4.2], [6, 7.3]], // example index 1
      ]
    ), ['batch', 'pos', 'repSize']);
    ...
    const expectedGTensor = new gtensor.GTensor(tf.tensor([
  ...
  it('attentionHead1', () => {
    ...
    const batchedInput = gtensor.makeTruncNormal(
      { batchSize: 10, seqLen: 8, inputRep: 2 });

    function attentionHeadFn(input: GTensor<'seqLen' | 'inputRep'>)
      : GTensor<'seqLen' | 'valueRep'> {
      const inputKeys =
        input.contract(keyM, ['inputRep']).rename('seqLen', 'keySeqLen');
      ...
      const attendedValues = values.contract(
        attention, ['seqLen']).rename('keySeqLen', 'seqLen');
  ...
  xit('attentionHead2', () => {
    ...
    type ExactGTensor<Exact extends string, Given extends string> =
      Exclude<Given, Exact> extends never ? GTensor<Given>
      : ErrorGivenHadExtraTypes<Exclude<Given, Exact>>;

    function attentionHeadFn2<T extends string>(
      maybeInput: ExactGTensor<'seqLen' | 'inputRep', T>): GTensor<'seqLen' | 'valueRep'> {
      const input = maybeInput as never as GTensor<'seqLen' | 'inputRep'>;
      const inputKeys = input.contract(
        keyM, ['inputRep']).rename('seqLen', 'keySeqLen');
      const inputQueries = input.contract(queryM, ['inputRep']);
      const attention = inputKeys.contract(inputQueries, ['kqRep']);
      const values = input.contract(valueM, ['inputRep']);
      const attendedValues = values.contract(attention, ['seqLen'])
