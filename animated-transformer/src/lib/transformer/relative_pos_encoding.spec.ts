...
describe('relative_pos_encoding', () => {
  it('Basic Relative position encoding', () => {
    ...
    const posEncoding = gtensor.makeRange('pos', -1 * (seqLength * 2 - 1), 0, 1, 'int32');
    ...
    const keyPos = gtensor.makeRange('keyPos', 0, seqLength, 1, 'int32');
    const queryPos = gtensor.makeRange('queryPos', 0, seqLength, 1, 'int32');
    ...
    expect(posMatrix.dimNames).toEqual(['keyPos', 'queryPos']);
  });
});
